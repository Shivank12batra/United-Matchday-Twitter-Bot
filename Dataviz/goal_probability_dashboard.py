...
def goal_probability_plot(goals, ax, team_name, team_color):
    ...
    goals_count = {i: k for i, k in sorted(
        goals_count.items(), key=lambda x: x[0])}
...
#     goals_count = Counter(goals)
    ...
    if m[1] - m[0] <= 5:
        ...
        x_text = -0.6
        y_text = max(prob) + 8
#     if int(str(max_num).split('.')[0][1]) > 5:
#         x_text = -0.6
#         y_text = max(prob) + 6.5
#     else:
#         x_text = -0.8
#         y_text = max(prob) + 5
    plt.text(x_text, y_text, f'{team_name} Goal Probability Distribution',
             fontweight='bold', fontfamily='serif', fontsize=20)
...
def horizontal_bar(away, draw, home, ax, h_team, a_team, a_color, h_color):
    ...
    plt.text(0, 2.6, 'Result Probability', fontweight='bold',
             fontfamily='serif', fontsize=18)


def scoreline_probability(scorelines, actual):
    s2 = Counter(scorelines)
    s2 = {i: k for i, k in sorted(
        s2.items(), key=lambda x: x[1], reverse=True)}
...
def goal_probability_dashboard(shots_data):
    ...
    ax.text(-0.15, 1.55, 'GAME SHOTS SIMULATION', fontsize=29,
    ...
    HighlightText(-0.15, 1.2, f'Actual scoreline: <{final[2]} ({final[3]}%) {text}>', size=16, fontweight='bold',
                  fontfamily='serif', highlight_textprops=[{'color': 'grey'}], ax=ax)
    plt.savefig('images/goal_probability_dashboard_twitter_bot.png',
